    ...
    case block of
        ...
        Docs.UnknownBlock name ->
            span
                ...
                , text " with the title UnkownBlock found in docs and with a link to this page in description."
                ]
    ...
    case lines of
        One width line ->
            ...
            if potentialWidth < maxWidth then
                ...
                OneOrMore [ text field, space, colon ] []
...
toMoreLines s x xs =
    let
        ...
        toChunk (OneOrMore y ys) =
            (s.sep :: y) :: List.map ((::) sepIndent) ys

        otherLines =
            List.map ((::) openIndent) firstRest
...
keyword kw =
    span [ class "hlds-keyword" ] [ text kw ]
